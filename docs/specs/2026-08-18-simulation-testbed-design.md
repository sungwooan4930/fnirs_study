...
`run_id` = `<run_name>_seed<N>_<git_short_hash>_cfg<config_hash8>`

`config_hash8`은 **오버라이드가 병합된 실효 config**(단 `output` 제외)의 sha256 앞 8자다. 이것이 없으면 오버라이드로 조건만 바꾼 실행들이 전부 같은 디렉토리를 가리키고 `mkdir(exist_ok=True)`가 조용히 통과해 `metrics.json`이 차례로 덮어써진다 — T4 스윕 6개 조건을 기본 `results/`에 돌리면 5개가 파괴되고 마지막 하나만 남는데, 남은 디렉토리는 내부적으로 일관되어 아무 이상도 드러나지 않는다. `output`을 해시에서 빼는 이유: 어디에 저장하느냐는 실험의 정체성이 아니다.
...
#### ⚠ T3가 실제로 증명한 것 / 증명하지 않은 것 (2026-08-19 정정)

측정된 부풀림은 15%p가 아니라 **7.55%p**였다 (LOSO 0.6542 → `window_random` 0.7297).
Task 20의 분석은 그 잔차가 **주로 subject-identity 누수**라고 결론지었다 —
`pilot.yaml`은 블록이 60초라 한 블록 안에 5초/1초 창이 약 52개 들어가고 전부 같은
라벨을 공유하므로, 인접 창 누수의 대부분은 이미 합법적인 블록 수준 신호를 중복한다.

그 분석이 옳다면 **지금 구성된 T3는 "피험자 혼합이 정확도를 부풀린다"를 증명한 것이지
"80% 창 오버랩이 정확도를 부풀린다"를 증명한 것이 아니다.** 그런데 창 오버랩이야말로
`CLAUDE.md` §5.1이 이 프로젝트 최대 리스크로 지목한 위험이고, 이 절이 "존재 이유"로
내건 대상이다. 확정된 것은 다음 둘로 한정된다:

1. 피험자 중첩 가드가 `window_random` 분할을 실제로 차단한다 (T3a).
2. 피험자를 섞으면 정확도가 **최소** 7.55%p 부풀려진다 (T3b).

**결정적 실험은 서브프로젝트 B로 이월한다.** 피험자 정체성을 상수로 고정한 채
(= 각 피험자 **내부에서** 창을 무작위 분할하는 분할기) 창-오버랩 누수만 분리해
측정한다. 그때 블록 길이를 줄이거나 블록 내 라벨이 변하는 설계를 함께 써야
창 인접성이 블록 수준 신호와 분리된다. 위 표의 T3 판정 문턱과 테스트는 이 정정으로
**변경하지 않았다** — 측정치가 틀린 것이 아니라, 그것이 무엇에 대한 증거인지의
해석이 좁혀진 것이기 때문이다.
...
| 특정 fold에서 클래스가 1개뿐 | 실행 거부 — 정확도가 무의미해짐. **train fold와 test fold 양쪽** 모두 검사한다 (test가 단일 클래스면 그 fold의 정확도는 판별력이 아니라 다수 클래스 예측률이다) |
...
4. **T2의 구체적 정확도 범위** — 확정. `config/experiments/pilot.yaml`(EEG 1000 Hz, 12명, effect_size=0.8, seed=42)
   ...
   (허용 오차 ±0.005). Task 19에서 측정. 2026-08-19 fix wave 이후에도 비트 단위로 동일 —
   베이스라인 재생성 불필요.

### 11.5 승인 기준의 현재 상태 (2026-08-19 fix wave 반영)

**T1·T2·T4는 통과했고, T3는 부분 통과다.** §8의 정정 박스를 함께 읽을 것 —
T3는 subject-identity 누수를 증명했고 **창-오버랩 누수는 아직 증명하지 않았다**.
"T1~T4 전부 통과 = A+D 완료"라는 이전 서술은 창-오버랩 누수가 정답 대비 시연됐다는
뜻으로 읽히므로 여기서 정정한다.

fix wave 이후 실측 (config `pilot.yaml`, 시드 42, n=6000 창/실행, **chance 0.3333**,
원본 `.superpowers/sdd/2026-08-18-simulation-testbed/fixwave-measurements.txt`):

| 검증 | CV 방식 | 수치 | 비고 |
|---|---|---|---|
| T1 널 | loso | pooled 0.3273, 95% CI [0.3155, 0.3394], p=0.331 | chance 포함 — 통과 |
| T2 효과 회복 | loso | pooled 0.6542, worst 0.2960 (`sub-05`) | fix wave 전후 불변 |
| T3 누수 | window_random (가드 OFF) | pooled 0.7297, 부풀림 **7.55%p** | 부분 통과 — 위 정정 박스 참조 |
| T4 τ²=0 / 0.5 / 2.0 | loso | 0.9997 / 0.6542 / 0.6225 | 단조 하락 — 통과 |
| T4 τ²=0 / 0.5 / 2.0 | within_subject | 0.9927 / 0.9755 / 0.9870 (worst 0.9702 / 0.8274 / 0.9107) | 견고 — 통과 |

**LOSO 행과 within_subject 행을 혼용해 인용하지 않는다** (`CLAUDE.md` §5.4).

fix wave에서 이동한 수치와 그 원인:
- T4 LOSO τ²=0: 1.0000 → **0.9997**. `subject_variance=0` 분기가 `rng.normal`을 건너뛰어
  생성기 상태를 소모하지 않았고, 그 결과 τ²=0 조건만 다른 난수열 위에서 돌고 있었다.
  분기를 제거해 단일 변수 조작을 회복했다.
- T4 within_subject 전체: 0.9312 / 0.8827 / 0.8948 → **0.9927 / 0.9755 / 0.9870**,
  `accuracy_worst` 0.0000 → **0.9702 / 0.8274 / 0.9107**. `GroupKFold(3)`가 셔플된 9블록을
  인덱스 stride로 잘라 test fold에 {i, i+3, i+6}을 넣었고, 그 셋이 같은 수준이면 —
  수준당 블록이 3개뿐이므로 — train에 그 수준이 전무하고 test가 단일 클래스가 되어
  정확도가 구조적으로 0이 됐다(36 fold 중 3개). 블록을 라벨별로 fold에 라운드로빈
  배정하는 **층화**로 해소. 단일 클래스 fold는 이제 train·test 양쪽에서 거부한다(§9).

---

## 12. 성공 기준

- [x] T1·T2·T4 통과 / **T3는 부분 통과** — 가드 차단(T3a)과 subject-identity 누수로 인한
      부풀림(T3b)은 증명, **창-오버랩 누수는 미증명**(§8 정정 박스·§11.5). 결정적 실험은 B로 이월
