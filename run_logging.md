...
**후속 판단 사항으로 명시한다(B1 범위 밖):** 60초에서도 T2·T3가 성립하도록
드리프트 시그마를 재보정할지, 또는 노출 감소를 통제한 재실험으로 두 원인을
분리할지는 이 리뷰가 결정하지 않는다. config는 이 리뷰에서 바꾸지 않았다.
...
- [x] ~~`session_recovery.yaml`·`session_clean.yaml`(T2·T3·T4)이 여전히
      ...
      않는다.
- [ ] (신규) 60초에서도 T2·T3가 성립하도록 드리프트 시그마를 재보정할지, 또는
      세션 길이를 고정한 채 원인을 분리하는 재실험을 할지 — 연구책임자/후속
      작업 판단 사항. B1 범위 밖.
