...
### 2026-08-19 — Task 16 두 번째 후속: T6 판정을 비율로, 임계를 전 config에 통일

컨트롤러가 정정 6의 "간격 0.024" 잔여 실패를 재검토해 **판정 기준 자체가
브리프 원안보다 과했다**고 결론지었다 — "핵심 음성 칸의 모든 피험자가
양성 칸의 모든 피험자보다 낮을 것"은 개념(민감도·특이도)이 요구하는 것보다
엄격하다.

#### 완료
- T6 메인 테스트를 "개별 녹화 완전 분리"에서 "칸별 플래그 비율"로 재설계.
  `T6_NEGATIVE_FLAG_RATE_MAX=0.25`·`T6_POSITIVE_FLAG_RATE_MIN=0.75`(3:1
  변별 사전 선언), 실패 메시지에 네 칸 실측 비율 전부 출력
- `xfail` 제거 — 정상 테스트로 통과
- `session_quality.yaml`의 `n_subjects`를 4 → 8로 올림(비율 추정 세분화).
  드리프트 시그마는 건드리지 않음
- `drift_threshold_relative: 0.20`을 쓰던 나머지 5개 config(`smoke`·`null`·
  `pilot`·`session_recovery`·`session_clean`)를 전부 `2.15`로 일괄 갱신
- `null.yaml` 재확인: `n_sessions_flagged` 36/36 → **12/36**로 정상 범위 진입

#### 결정 사항
| 항목 | 결정 | 근거 | 대안 (기각 사유) |
|------|------|------|-----------------|
| T6 판정 방식 | 완전 분리 → 비율(민감도·특이도) | 브리프 원안이 개념보다 엄격한 기준을 요구했다(컨트롤러 판정) | n_subjects를 더 늘려 완전 분리를 시도(꼬리 확률상 여전히 실패 가능 — 컨트롤러가 명시적으로 기각) |
| 임계 통일 범위 | Task 16 config 전부(6개) | 지표가 스케일 자유이므로 config별로 다를 이유가 없다(컨트롤러 지시) | session_quality.yaml만 유지(다른 config의 36/36 문제를 방치하게 됨 — 미채택) |

**T6 실측 (n=8, 임계 2.15):** 세션0=0.250(≤0.25 PASS) · 세션1=1.000(≥0.75
PASS) · 세션2(핵심 칸)=0.125(≤0.25 PASS) · 세션3=1.000(≥0.75 PASS). **전부
PASS, `xfail` 불필요.**

**null.yaml 재측정:** eeg 범위 0.264~4.521(임계 위아래에 걸침), fnirs 범위
0.188~0.327(전부 임계 아래) — 남은 12/36 플래그는 전부 eeg가 낸 것이고,
`assignment: sampled`에서 우연히 큰 이득을 뽑은 세션들로 보인다. 포화가
풀렸다는 것은 애초의 문제가 "지표가 뭔가를 오독한다"가 아니라 "임계가
낡았다"였다는 컨트롤러의 판정을 뒷받침한다.

#### 계획서 연계
목표 3 — `SessionQuality.drift_flag`가 이제 (1) 구조적 발산 없음(정정 6),
(2) 판정 기준이 개념에 맞음(정정 7), (3) 프로젝트 전체에서 같은 임계·같은
단위를 씀, 세 가지를 모두 만족한다. T6 승인기준이 처음으로 정상(non-xfail)
PASS 상태에 도달했다.

#### 미해결
- [ ] 다른 시드에서도 T6 비율 기준(0.25/0.75)이 성립하는지 재현 확인 —
      이번 실측은 seed 42 1회다.
- [ ] `null.yaml`의 남은 12/36 플래그가 실제로 "정상"인지(사전에 기대한
      비율과 비교할 기준이 없다), 아니면 `assignment: sampled`의 드리프트
      시그마 자체가 여전히 큰지는 컨트롤러 판단으로 남겨뒀다.

상세: `docs/specs/2026-08-19-session-baseline-design.md` §8.4.1 "정정 7",
`.superpowers/sdd/2026-08-19-session-baseline/task-16-report.md`.

### 2026-08-19 — Task 16 세 번째 후속: 정정 6·7의 원인 진단이 틀렸다 — 판정 B를 되돌린다

리뷰가 정정 6·7의 원인 진단("`n_subjects=4`의 소표본 피험자 변동")을
반증했다. 세 시드(42·7·2026)로 직접 측정한 결과 세션0·2의 오탐 비율이
0.125~0.500으로 시드마다 흔들렸다 — 소표본 변동이라면 시드 무관하게
비슷한 패턴이어야 하는데, 흔들림 자체가 추정 절차의 불안정성을 가리킨다.
**진짜 원인은 `baseline_duration_s`(베이스라인 창 수)였다.** 20초
베이스라인은 창을 16개만 주고, 정정 6이 §6.3 분모를 `std(ddof=1)`로 바꾼
것 때문에(그 수정 자체은 옳다) 창이 적으면 SD 추정이 불안정해져 48채널
fnirs가 그 잡음을 그대로 받는다. `baseline_duration_s`를 60(창 ~56개)으로
늘리면 세 시드 전부 완전 분리로 돌아온다.

**판정 A(§6.3 분모를 평균→SD)는 유지, 판정 B(T6을 비율 판정으로 완화)는
철회한다.** 판정 B는 "고칠 수 있는 측정 아티팩트를 판정 기준 완화로
흡수"한 것이었다 — 정직하게 xfail로 남기고 원인을 보고했던 Task 16의
1차 판단이 옳았다.

#### 완료
- `config/experiments/session_quality.yaml`의 `baseline_duration_s`를
  20 → 60(이 config만. `session_recovery`·`session_clean`은 T2·T3가
  이미 통과 중이라 건드리지 않음)
- T6 메인 테스트를 완전 분리 기준(브리프 원안)으로 복원. 비율 상수
  (`T6_NEGATIVE_FLAG_RATE_MAX`·`T6_POSITIVE_FLAG_RATE_MIN`) 삭제. 실패
  메시지에는 네 칸의 플래그 목록·실측 드리프트 값을 계속 출력(진단용)
- `n_subjects=8`은 유지(되돌릴 이유 없음 — 표본이 많을수록 완전 분리
  요구가 더 엄격한 시험이 된다)
- `test_t6_injection_zero_threshold_destroys_specificity`(거의 항진명제)를
  폐기하고 `test_t6_injection_blind_to_end_baseline_misses_the_positive_
  cells`로 교체 — `compute_drift`가 종료 베이스라인을 못 보게 만들어(시작
  베이스라인을 자기 자신과 비교) 양성 칸(세션1·3)이 100%→0%로 무너지는
  것을 확인하는 진짜 결함 주입
- `docs/specs/2026-08-19-session-baseline-design.md` §8.4.1에 "정정 8"
  추가 — 정정 6·7의 오진단을 명시적으로 정정하고 근거 표를 남김

#### 결정 사항
| 항목 | 결정 | 근거 | 대안 (기각 사유) |
|------|------|------|-----------------|
| baseline_duration_s 변경 범위 | `session_quality.yaml`만 | 다른 config(`session_recovery`·`session_clean`)는 T2·T3가 통과 중이라 건드리면 그 결과가 흔들린다(컨트롤러 지시) | 전 config 일괄 변경(범위 이탈, 무관한 결과를 흔들 위험 — 미채택) |
| T6 판정 방식 | 비율 → 완전 분리로 되돌림 | 리뷰가 잔여 실패의 진짜 원인(베이스라인 창 수)을 밝혀 판정 완화가 불필요했음을 보임 | 비율 유지(원인을 고쳤는데도 기준을 낮춘 채로 두는 것 — 미채택) |

**세 시드 재현 (session_quality.yaml, baseline_duration_s=60, n_subjects=8, 임계 2.15):**

| seed | s0 | s1 | s2(핵심) | s3 | 완전 분리 |
|---|---|---|---|---|---|
| 42 | 0.000 | 1.000 | 0.000 | 1.000 | 예 |
| 7 | 0.000 | 1.000 | 0.000 | 1.000 | 예 |
| 2026 | 0.000 | 1.000 | 0.000 | 1.000 | 예 |

세 시드 전부 완전 분리(세션0·2 전원 비플래그, 세션1·3 전원 플래그)로
재현됐다.

#### 계획서 연계
목표 3 — §6.3 `drift_flag`가 이제 (1) 구조적 발산 없음(판정 A), (2)
브리프 원안 그대로의 엄격한 판정을 세 시드에서 완전히 만족(판정 B 철회
후), (3) 프로젝트 전체 임계 통일, 세 가지를 모두 만족한다. **베이스라인
길이가 드리프트 지표의 신뢰도를 결정한다는 발견은 실장비 프로토콜
설계(CLAUDE.md §2.4·§3.8의 세션 시작·종료 베이스라인 측정 시간)에 직접
반영해야 한다** — 최소 베이스라인 길이(또는 최소 창 수) 하한을 정하는
근거로 이번 관측(16개 창 불안정 vs 56개 창 안정)을 쓴다.

#### 미해결
- [ ] CLAUDE.md·연구 프로토콜 문서에 "베이스라인 측정 시간의 최소 하한"을
      명시적으로 반영할지 결정 필요(연구책임자 판단)
- [ ] `null.yaml`의 남은 12/36 플래그가 실제로 "정상"인지 — 아직 판단
      기준이 없다
- [ ] `session_recovery`·`session_clean` 등 `baseline_duration_s=20`을
      쓰는 나머지 config도 같은 SD 추정 불안정성을 겪을 수 있으나, 이번
      정정 범위 밖이라 손대지 않았다 — 후속 조사 필요
...
**2026-08-19 최종 리뷰 추기 — 위 "T1~T6 전부 PASS"는 `baseline_duration_s=20`
에서의 결과다.** 이 브랜치가 T6에서 신뢰할 수 있다고 스스로 결론지은 60초에서는
T2·T3가 FAIL한다. `session_recovery.yaml`을 `baseline_duration_s`만 20→60으로
바꿔 seed 42로 재실행한 실측:
...
드리프트를 전부 0으로 끈 `session_clean.yaml`도 20초에서 24개 세션 중 3개를
`drift_flag`로 오탐한다(fnirs 집계 최대 3.750 > 임계 2.15) — 60초에서는 0/24다.

**원인:** `compute_drift`의 분모이자 `SessionBaseline.fit`의 `scale`이 같은
σ̂(베이스라인 산포 추정치)다. T6은 "20초는 σ̂ 추정을 불안정하게 만든다"고
결론짓고 `session_quality.yaml`만 60초로 올렸는데, T2·T3·T4가 쓰는
`session_recovery.yaml`·`session_clean.yaml`은 여전히 20초다 — 즉 **같은
불안정한 σ̂ 위에서 T2·T3·T4를 실행해놓고 PASS로 기록한 것**이며, 브랜치는
내부적으로 어긋난 상태로 병합된다.

**교란요인 (분리되지 않음):** 베이스라인을 늘리면 세션 총 길이가 220s→300s가
되어, 세션 내 드리프트 램프 중 과제 블록이 겪는 구간이 `rate·180/220` →
`rate·180/300`으로 줄어든다. 실제 드리프트 노출 자체가 약해지므로 T2의 "붕괴"
조건이 덜 성립한다. σ̂ 불안정성과 이 노출 감소는 이 측정만으로 분리되지 않는다.

**후속 판단 사항이었고, 2026-08-20 사용자가 결정했다: 20초를 유지한다.**
60초 재보정도, 노출 감소를 통제한 재실험도 하지 않는다. config는 바꾸지 않았다
(`session_recovery.yaml`·`session_clean.yaml`은 `baseline_duration_s=20`으로 출하).
따라서 T2·T3의 PASS는 **T6가 요구하는 것보다 짧은 베이스라인 위의 결과**이며,
그 한정을 인용 시 함께 옮긴다. 상세는 아래 "미해결" 절.
상세: `docs/specs/2026-08-19-session-baseline-design.md` §12 항목 10.

### 미해결

- [x] ~~`session_recovery.yaml`·`session_clean.yaml`(T2·T3·T4)이 여전히
      `baseline_duration_s=20`을 쓴다 — T6과 같은 SD 추정 불안정성의 영향을
      받을 수 있으나 **확인하지 않았다**. T2·T3·T4를 60초로 재실행해 결과가
      바뀌는지 확인이 필요하다.~~ **2026-08-19 최종 리뷰에서 실측 완료.** 60초에서
      T2·T3 FAIL, T4는 PASS(위 "계획서 연계" 표 참조). 세션 길이 변화(220s→300s)라는
      교란요인이 있어 원인이 σ̂ 불안정성만인지 노출 감소도 섞였는지는 분리되지
      않는다.
- [x] ~~(신규) 60초에서도 T2·T3가 성립하도록 드리프트 시그마를 재보정할지, 또는
      세션 길이를 고정한 채 원인을 분리하는 재실험을 할지~~ — **2026-08-20 사용자
      결정: 20초를 유지한다.** `session_recovery.yaml`·`session_clean.yaml`을
      바꾸지 않는다.

      **이 결정이 뜻하는 것.** T2·T3의 PASS는 유지되지만, 그 실행은 T6가
      "드리프트 지표를 신뢰하려면 필요하다"고 결론지은 것보다 **짧은 베이스라인
      위에서 돈다**(20초 = 창 약 16개, 80% 오버랩이라 독립 관측은 실질 ~4개).
      같은 σ̂가 `compute_drift`의 분모이자 `SessionBaseline.fit`의 `scale`이므로,
      T2·T3의 정규화는 T6가 요구하는 것보다 잡음 많은 σ̂ 위에 있다.
      **미해결이라서가 아니라 선택해서 이 상태다.** B1의 T2·T3 수치를 인용할 때는
      이 한정을 함께 옮긴다.

      실장비 프로토콜은 별개다 — `CLAUDE.md` §2.4가 매 세션 베이스라인을
      의무화하되 **길이 하한을 정하지 않았고**, 위 사실은 그 하한이 드리프트 지표의
      신뢰도를 결정한다는 것을 보인다. 실측정 프로토콜 확정 시 재검토 대상.
