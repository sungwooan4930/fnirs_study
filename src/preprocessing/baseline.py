...
def compute_drift(
    ...
    **2026-08-19 정정 (Task 16 — 컨트롤러 판정, T6 실측이 드러낸 결함).**
    분모를 시작 베이스라인의 **평균**(`|mean(start)|`)이 아니라 **산포**
    (`std(start, ddof=1)`)로 바꿨다. 초판은 fnirs 특징 벡터가 `[HbO 평균 |
    HbO 기울기]`를 이어붙인다는 것을 몰랐다 — "기울기"(창 내 선형 추세)
    특징은 베이스라인 구간(고정점 응시, 추세 없음)에서 평균이 **정당하게
    0 근처**다. 평균을 분모로 쓰면 이 채널들이 "수치적으로 0"과 구분되지
    않아 비율이 잡음 수준으로 발산한다 — `zero_atol`(1e-8)은 "정확히 0"만
    걸러내는 수치적 하한이라 이런 채널을 하나도 제외하지 못했다(T6 실측:
    `n_excluded == 0`인 채로 세션마다 무관하게 큰 값이 나옴, 원인은
    §6.3 정의 자체가 아니라 이 분모 선택이었다).

    산포를 쓰면 세 가지가 함께 해결된다: (1) 평균이 0이어도 산포는
    0이 아니므로 기울기 특징도 정상적으로 다뤄진다. (2) §6.2
    `concentration_delta`(`SessionBaseline`)가 이미 같은 베이스라인 산포로
    나누므로, 드리프트가 "세션이 베이스라인 표준편차 몇 개만큼 움직였나"가
    되어 정규화와 **같은 단위**를 쓰게 되고 서로 다른 특징(평균 vs 기울기,
    eeg vs fnirs) 간 크기 비교가 비로소 의미를 갖는다. (3) `zero_atol`이
    원래 의도대로 동작한다 — 산포가 **진짜로 0인**(값이 창마다 상수인) 채널만
    제외되고, 제외 개수가 "이 채널은 평가할 수 없다"는 실제 정보가 된다.

    시작 베이스라인 창이 1개뿐이면 표본 표준편차(ddof=1)가 정의되지 않는다
    (0/0). `SessionBaseline.fit`이 같은 경우를 다루는 방식과 동일하게 0으로
    시작해 아래 `zero_atol` 하한 배제가 곧바로 적용되도록 한다 — NaN을 만들어
    배제 로직을 무력화하지 않는다.

    산포가 수치적으로 0에 가까운(=사실상 상수인) 채널은 비율이 발산하므로
    ...
    if start.shape[0] >= 2:
        spread = start.std(axis=0, ddof=1)
    else:
        spread = np.zeros(start.shape[1])
